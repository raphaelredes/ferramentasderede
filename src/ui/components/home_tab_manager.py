# app/ui_components/home_tab_manager.py
...
from ...network.discovery import DiscoveryScanner
...
class HomeTabManager:
    def __init__(self, parent_tab, app):
        ...
        # Controlador único para todas as ferramentas de rede da aba Home
        self.home_tool_controller = NetworkToolController(self.app, host={'ip': ''}, hostname='')
        
        # Scanner e seus dados
    ...
    def _create_widgets(self):
        """Cria a estrutura de sub-abas dentro da aba Home."""
    ...
    def _create_home_ping_tab(self, parent_tab):
        ...
        self.home_ping_output_textbox = customtkinter.CTkTextbox(parent_tab, font=("Consolas", 12), height=300)
        self.home_ping_output_textbox.grid(row=1, column=0, sticky="nsew", padx=10, pady=(0, 10))
    ...
    def _create_home_traceroute_tab(self, parent_tab):
        ...
        self.home_traceroute_output_textbox = customtkinter.CTkTextbox(parent_tab, font=("Consolas", 12), height=300)
        self.home_traceroute_output_textbox.grid(row=1, column=0, sticky="nsew", padx=10, pady=(0, 10))
    ...
    def toggle_home_ping(self):
        if self.home_tool_controller.running_command == f"ping_{self.home_ping_target_entry.get()}":
    ...
    def toggle_home_traceroute(self):
        if self.home_tool_controller.running_command == f"traceroute_{self.home_traceroute_target_entry.get()}":
            self.home_tool_controller.stop_command()
        else:
    ...
    def update_home_buttons_state(self):
        ...
        if ping_running:
            self.home_ping_button.configure(text=self.app.translate("ping_scan_stop"), state="normal")
        else:
            self.home_ping_button.configure(text=self.app.translate("ping_scan_start"), state="disabled" if traceroute_running else "normal")

        if traceroute_running:
            self.home_traceroute_button.configure(text=self.app.translate("ping_scan_stop"), state="normal")
        else:
            self.home_traceroute_button.configure(text=self.app.translate("traceroute_start"), state="disabled" if ping_running else "normal")
    ...
    def _populate_scanner_control_tabs(self, tab_view):
        ...
        host_names = [self.app.host_tab_view.tab_manager._get_display_name(h) for h in self.app.favorites]
        ...
        net_label = customtkinter.CTkLabel(custom_tab, text=self.app.translate("scanner_custom_net_address"))
        net_label.grid(row=0, column=0, padx=(10,5), pady=(10,0))
        ...
        mask_label = customtkinter.CTkLabel(custom_tab, text=self.app.translate("scanner_custom_subnet_mask"))
        mask_label.grid(row=1, column=0, padx=(10,5), pady=5)
    ...
    def _prepare_for_scan(self):
        self._clear_scanner_results()
        self._set_scanner_buttons_state(is_scanning=True)
        self.scanner_add_button.configure(state="disabled")
        self.scanner_progressbar.start()
    ...
    def _start_remote_scan(self):
        ...
        target_host = next((h for h in self.app.favorites if self.app.host_tab_view.tab_manager._get_display_name(h) == host_name), None)
    ...
    def _on_scan_finished(self):
        ...
        self.scanner_progressbar.stop()
        self.scanner_progressbar.set(1)
    ...
    def _add_host_to_scanner_results(self, host_data):
        ...
        container = self.scanner_domain_groups[domain]
        ...
        self.scanner_checkbox_widgets[ip] = checkbox
        self.scanner_add_button.configure(state="normal")
    ...
    def _clear_scanner_results(self):
        ...
        self.scanner_domain_groups.clear()
        self.scanner_checkbox_widgets.clear()
        self.scanner_all_found_hosts.clear()
        self._show_scanner_placeholder_tab(self.app.translate("scan_placeholder_tab"))
    ...
    def _confirm_scanner_selection(self):
        ...
        for ip, cb in self.scanner_checkbox_widgets.items():
            if cb.winfo_ismapped() and cb.get() == 1:
                ...
                selected_hosts.append({"name": full_hostname, "ip": ip_from_text})
                cb.deselect()
