...
class HostTabManager:
    ...
    def _store_button_reference_and_update_status(self, host_name_key, display_name):
        # Para o MultiRowTabView, obter o botão diretamente
        if hasattr(self.tab_view, '_tabs') and display_name in self.tab_view._tabs:
            button = self.tab_view._tabs[display_name].get("button")
            if button and host_name_key in self.host_tabs_data:
        ...
        # Fallback para CTkTabview original se ainda estiver sendo usado
        elif hasattr(self.tab_view, '_segmented_button') and self.tab_view._segmented_button.winfo_exists() and display_name in self.tab_view._segmented_button._buttons_dict:
