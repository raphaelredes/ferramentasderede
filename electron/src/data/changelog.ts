...
export const CHANGELOG: ChangelogEntry[] = [
    ...
    },
    {
        ...
        changes: [
            { kind: 'feat', text: 'A aba Ferramentas foi reorganizada em CATEGORIAS (Diagnóstico, Descoberta, DNS & Nomes, Banda & Web, Infra & Cálculo) — navegação em dois níveis para acomodar as 16 ferramentas sem poluir a tela.' },
            { kind: 'feat', text: 'Nova ferramenta "TCP Ping": testa se um host está vivo (e mede latência) abrindo uma conexão TCP numa porta — funciona mesmo quando o host BLOQUEIA ICMP, que é o padrão do firewall do Windows em domínio AD. Aceita "Sair pela rede" (multi-VLAN).' },
            { kind: 'feat', text: 'Nova ferramenta "Path MTU": descobre o maior pacote que atravessa o caminho até um alvo sem fragmentar. Ajuda a diagnosticar "MTU black hole" de VPN/túnel/VLAN — causa clássica de "conecta mas trava em transferências grandes".' },
            { kind: 'feat', text: 'Nova ferramenta "PTR Sweep": resolve o nome (DNS reverso) de TODOS os IPs de um CIDR de uma vez — inventário rápido de uma VLAN. Respeita o servidor DNS cadastrado por rede (multi-domínio).' },
            { kind: 'feat', text: 'Nova ferramenta "Certificado TLS": conecta em host:porta e mostra validade, dias até expirar, SAN, emissor e versão TLS — para caçar certs internos vencendo em appliances/intranet. Funciona com certs auto-assinados/internos.' },
            { kind: 'feat', text: 'Nova ferramenta "SNMP": lê dados de switches/roteadores via SNMP v1/v2c (nome, descrição, uptime, nº de interfaces) com community read-only. A ferramenta mais "corporativa" para quem opera VLANs.' },
            { kind: 'feat', text: 'Nova ferramenta "NTP": consulta o offset do relógio e stratum de um servidor de tempo. Skew de relógio quebra Kerberos — útil para diagnosticar falhas de autenticação em ambiente AD (alerta quando o offset passa de 5 min).' },
            { kind: 'feat', text: 'Nova ferramenta "HTTP": mede tempo de resposta (TTFB/total) e status de um endpoint web — separa lentidão de DNS/conexão/servidor ao checar um serviço de intranet.' },
            { kind: 'feat', text: 'Nova ferramenta "Sub-rede": calculadora de CIDR (rede, broadcast, faixa usável, máscara, wildcard) com divisão em sub-redes. Conveniência diária para quem vive em multi-VLAN.' },
            { kind: 'feat', text: 'Nova ferramenta "Conexões": lista conexões TCP/UDP ativas com o processo e PID dono (netstat -ano com nome de processo), com filtro por IP/porta/processo.' },
            { kind: 'fix', text: 'Correção do endpoint legado /network/status, que chamava um método inexistente (check_host_status_detailed) e retornava 500 — agora delega corretamente para o módulo de ping.' },
            { kind: 'refactor', text: 'O aplicativo passou a ser 100% baseado em bibliotecas de licença PERMISSIVA. A dependência icmplib (LGPL-3.0), antes usada por Ping/Traceroute e MTR, foi REMOVIDA — todas as funções de ICMP agora usam o ping/tracert nativos do Windows. O MTR foi reescrito para medir cada salto com ping nativo. Avisos completos de terceiros empacotados em THIRD-PARTY-LICENSES.txt.' },
            { kind: 'feat', text: 'Nova aba "MTR" (Path Monitor): combina ping + traceroute num monitor contínuo que mostra perda, latência (último/média/melhor/pior) e jitter SALTO A SALTO, atualizando ao vivo. É a melhor visão para descobrir EM QUE PONTO da rota até uma VLAN a latência ou a perda aparece. Aceita "Sair pela rede" (multi-VLAN). Usa o tracert e o ping nativos do Windows — sem binário externo nem bibliotecas de licença restritiva.' },
            { kind: 'feat', text: 'Nova aba "Portas": teste de portas TCP num host — lista (80, 443, 3389) ou faixa (8000-8100), além de um botão "Portas comuns" que varre as portas mais usadas. Saída streaming destacando as portas abertas.' },
            { kind: 'feat', text: 'Nova aba "DNS": consulta direta (nome→IP) e reversa (IP→nome) com detecção automática pelo formato do valor. Permite escolher o SERVIDOR DNS de uma VLAN/domínio específico — essencial em ambiente multi-AD onde o mesmo nome curto resolve diferente em cada domínio. Histórico das últimas consultas na tela.' },
            { kind: 'feat', text: 'Nova aba "Tráfego": taxa de transferência (download/upload) em tempo real POR INTERFACE de rede, via psutil. Bom para enxergar quanto passa por cada NIC numa máquina multi-VLAN. Amostragem a cada 1s com cálculo de taxa a partir dos contadores acumulados.' },
            { kind: 'feat', text: 'Nova aba "Banda (iPerf)" em Ferramentas para medir vazão (Mbits/s) em redes locais e VLANs. Dois modos: SERVIDOR (este PC vira o destino — você inicia o servidor e os outros pontos da rede rodam o cliente apontando para cá) e CLIENTE (testar a banda deste PC até um servidor iperf já existente na rede).' },
            { kind: 'feat', text: 'Modo Servidor mostra, pronto para copiar, o comando exato que o pessoal do outro lado deve rodar ("iperf -c <IP-deste-PC> -p 5201") — já preenchido com o IP da rede selecionada. Saída ao vivo de cada conexão e da banda medida.' },
            { kind: 'feat', text: 'Modo Cliente com opções de duração do teste, "Reverso" (servidor envia / este PC recebe — mede o sentido de download) e UDP, além de TCP. Saída streaming em tempo real, igual ao Ping.' },
            { kind: 'feat', text: 'Multi-VLAN: tanto o servidor quanto o cliente aceitam "Sair/Escutar pela rede" (bind na NIC de origem via -B), espelhando o seletor de source IP do Ping/Traceroute. Em máquina com NICs em várias VLANs, o teste sai/escuta pela interface certa.' },
            { kind: 'feat', text: 'Usa o iPerf2 nativo do Windows empacotado no portátil (executável único, sem dependências externas). O iPerf3 foi descartado de propósito: no Windows ele sempre exige a cygwin1.dll (GPL, +3 MB) — o iPerf2 tem build nativo BSD limpo. Crédito e licença BSD adicionados na tela "Sobre".' },
            { kind: 'security', text: 'Endpoints /tools/iperf/server e /tools/iperf/client validam alvo (mesma regra do ping), porta (1–65535), source IP e duração (limitada a 60s). O binário é chamado por lista de argumentos (nunca shell), e o processo é encerrado de forma limpa ao parar/desconectar — sem deixar iperf órfão.' },
        ],
    },
    {
        version: '1.2.8',
