...
export function LastExecutionBadge({ timestamp, target, onClear, label }: LastExecutionBadgeProps) {
    ...
    try {
        ...
        if (!isNaN(d.getTime())) {
            formatted = d.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }) + ' - ' + d.toLocaleDateString();
        } else {
    ...
    return (
        <div className="flex items-center justify-between gap-2 px-3 py-1.5 rounded-lg bg-zinc-950/60 border border-zinc-800 text-xs text-zinc-400">
            <div className="flex items-center gap-1.5">
                ...
                <span>{label || 'Última execução salva'}:</span>
            ...
            {onClear && (
                <button
                    onClick={onClear}
                    type="button"
                    className="flex items-center gap-1 text-[11px] text-zinc-500 hover:text-red-400 transition-colors p-0.5 rounded"
